...
$( document).ready(function() {
    ...
    for(var i = 0; i < categories.length; i++) {
        for(var j = 0; j < categories[i].length; j++) {
            ...
            $("#" + categoryNames[i].toLocaleLowerCase() + "-dropdown-section > div > div").append('<input class="form-check-input" type="checkbox" value="" id="itemEntry' + i.toString() + '.' + j.toString() + '"><label id="checkboxItemLabel'.concat(i.toString()).concat(".").concat(j.toString())' + j.toString() + '" class="form-check-label" for="itemEntry' + i.toString() + '.' + j.toString() + '"></label><br>');
