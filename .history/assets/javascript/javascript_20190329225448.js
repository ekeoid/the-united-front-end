...
$( document).ready(function() {
    ...
    for(var i = 0; i < categories.length; i++) {
        for(var j = 0; j < categories[i].length; j++) {
            ...
            $("#" + categoryNames[i].toLocaleLowerCase() + "-dropdown-section > div > div").append('<input class="form-check-input" type="checkbox" value="" id="itemEntry' + Integer.toString(i) + '.' + Integer.toString(j) + '"><label id="checkboxItemLabel' + Integer.toString(i) + '.' + Integer.toString(j) + '" class="form-check-label" for="itemEntry' + Integer.toString(i) + '.' + Integer.toString(j) + '"></label><br>');
            // $("#" + categoryNames[i].toLocaleLowerCase() + "-dropdown-section > div > div > #checkboxItemLabel" + i + '.' +  j).text(categories[i][j]);
            $("#checkboxItemLabel" + Integer.toString(i) + "." +  Integer.toString(j)).html("<p>categories[i][j]</p>");
