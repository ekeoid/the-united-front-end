...
$( document).ready(function() {
    ...
    for(var i = 0; i < categories.length; i++) {
        for(var j = 0; j < categories[i].length; j++) {
            ...
            $("#" + categoryNames[i].toLocaleLowerCase() + "-dropdown-section > div > div").append('<input class="form-check-input" type="checkbox" value="" id="itemEntry' + i.toString() + '.' + j.toString() + '"><label id="checkboxItemLabel' + i.toString() + '.' + j.toString() + '" class="form-check-label" for="itemEntry' + i.toString() + '.' + j.toString() + '"></label><br>');
            // $("#" + categoryNames[i].toLocaleLowerCase() + "-dropdown-section > div > div > #checkboxItemLabel" + i + '.' +  j).text(categories[i][j]);
            $("#checkboxItemLabel" + i.toString() + "." +  j.toString()).html("<p>categories[i][j]</p>");
